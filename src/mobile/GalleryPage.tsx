...
export const GalleryPage: React.FC = () => {
  ...
  const list = useMemo(() => {
    ...
    return galleryComponents.filter(
      (item) => item.key.toLowerCase().includes(normalized) || item.title.toLowerCase().includes(normalized),
    );
  }, [keyword]);
      ...
      <div className='mk-gallery-card'>
        ...
        <div className='mk-gallery-body'>
          ...
          <div className='mk-gallery-list'>
            {list.map((item) => (
              <a key={item.key} href={item.route} className='mk-gallery-item'>
                <span>{item.title}</span>
                <span className='mk-gallery-item-arrow'>›</span>
              </a>
            ))}
            {list.length === 0 && <div className='mk-gallery-empty'>未找到匹配组件</div>}
          </div>
        </div>
      </div>
    </div>
